...
private let NetworkRequestShareInstance = NPNetWorkTools()
private let ServerUrl:String = "http://huihui.huasuhui.com:8022/hsh-api-portal/"
class NPNetWorkTools
{
    class var sharedInstance : NPNetWorkTools
    {
        return NetworkRequestShareInstance
    }
}

extension NPNetWorkTools
{
    //MARK: - GET 请求
    ...
    func getRequest(urlString: String, params : [String : Any], success : @escaping (_ response : [String : AnyObject])->(), failture : @escaping (_ error : Error)->()) {
        ...
        Alamofire.request(ServerUrl+urlString, method: HTTPMethod.get, parameters: params)
    ...
    {
        Alamofire.request(ServerUrl+urlString, method: HTTPMethod.post, parameters: params).responseJSON
            {
                (response) in
            switch response.result
            {
